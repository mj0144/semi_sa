...
<div id="colorlib-main">
	...
	<section class="ftco-section">
		<div style="margin-bottom: 30px; margin-left: 30px; font-size: 20px;">
			<a>천생연분 찾기</a> &nbsp;<span>/</span>&nbsp; <a href="listSome?code=2">나를
				도와주는 인연 찾기</a> &nbsp;<span>/</span>&nbsp; <a href="listSome?code=3">전체보기</a>
		</div>
		<div class="container">
							...
							<div class="block-27">
								<div style="display: block; text-align: center;">
									<c:if test="${paging.prev }">
										<a href="listSome?code=1&nowPage=${paging.startPage - 1 }">&lt;</a>
									</c:if>
									<c:if test="${paging.startPage != 1 }">
										<a
											href="listSome?code=1&nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}">&nbsp;</a>
									</c:if>
									<c:forEach begin="${paging.startPage }"
										...
										<c:choose>
											...
											<c:when test="${p != paging.nowPage }">
												<a
													href="listSome?code=1&nowPage=${p }&cntPerPage=${paging.cntPerPage}">${p }</a>
											</c:when>
										</c:choose>
									</c:forEach>
									<!-- 다음페이지 -->
									<c:if test="${paging.endPage != paging.lastPage}">
										<a
											href="listSome?code=1&nowPage=${paging.endPage+1 }&cntPerPage=${paging.cntPerPage}">&nbsp;</a>
									</c:if>
									<c:if test="${paging.next && paging.endPage > 0 }">
										<a href="listSome?code=1&nowPage=${paging.endPage + 1 }">&gt;</a>
									</c:if>
								</div>
							</div>
						</div>
